// 一次性脚本：生成全国所有地级行政区的空数据文件
// 用法：node scripts/generate-cities.mjs
...
for (const [province, cities] of Object.entries(regions)) {
  for (const city of cities) {
    const dir = join(root, province)
    const file = join(dir, `${city}.md`)
    if (existsSync(file)) {
      skipped++
      continue
    }
    mkdirSync(dir, { recursive: true })
    writeFileSync(file, template(province, city), 'utf8')
    created++
  }
}
